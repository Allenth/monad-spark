...
用户已要求先开发着陆页及 PPT 式产品演示，当前已实现。Sites/React 工程与依赖已建立；npm run build 通过。旧工作台保留 /workspace，未实现链上业务；合约测试仍是未完成工作。目标域名 captionrewrite.com 未绑定；MOJO 未提交。下一步按用户评审继续，不重新注册 Site 或 MOJO。页面与验收详见 docs/LANDING_AND_DEMO.md（相对项目根目录）。下方是此前交接历史，不能用“只有文档”判断当前状态。
